...
def shortcut_entries() -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("Click category", "Jump to first figure in that category (not checkbox)"),
        ("←", "Open root picker (when categories focused)"),
        ("↑ / ↓", "Move in category or root-picker list"),
        ("← / →", "Root picker: up directory / drill into folder"),
        ("Enter", "Root picker: open selected as scan root"),
        ("Esc", "Close root picker"),
        ("Tab", "Root picker: Nearby ↔ Recent"),
        (f"{mod}+← / {mod}+→", "First / last figure"),
        ("Space / Enter", "Next figure (or toggle category when list focused)"),
        ("P", "Play / pause video (when a video is showing)"),
        ("Scrub bar", "Seek within the current video"),
        ...
        (f"{mod}+C", "Copy current figure file to clipboard (when figure focused)"),
        ("Drag figure", "Drag source file out to other apps"),
